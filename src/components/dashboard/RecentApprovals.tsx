          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody className={`${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} divide-y divide-gray-200 dark:divide-gray-700`}>
              {recentApprovals.length === 0 ? (
                <tr>
                  <td colSpan={3} className={`px-3 sm:px-6 py-4 text-center text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                    Henüz onay işlemi bulunmuyor.
                  </td>
                </tr>
              ) : (
                recentApprovals.map((approval) => (
                  <tr key={approval.id} className={`${theme === 'dark' ? 'hover:bg-gray-700' : 'hover:bg-gray-50'} transition-colors duration-150`}>
